...
forward
global type u_cst_statictext from u_canvas
end type
...
//
Long    _Backcolor2
Boolean _VGradientFill = FALSE
Boolean _HGradientFill = FALSE
...
SetTextColor(ahdc, _FontColor)

//Drawtext(ahdc, #Text, LEN(#Text), l_Rect, ll_parm)
Drawtext(ahdc, _Text, LEN(_Text), l_Rect, ll_parm)
...
_VGradientFill = ab_vgradient

IF _VGradientFill THEN
	_HGradientFill = FALSE
END IF
...
//	#VGradientFill = FALSE
//END IF
_HGradientFill = ab_hgradient

IF _HGradientFill THEN
	_VGradientFill = FALSE
END IF
...
lstr_Lf.lffacename = _FontName
lstr_Lf.lfweight = 400
//lstr_Lf.lfheight = #FontSize * -1
lstr_Lf.lfheight = _FontSize * -1
...
lstr_Lf.lfescapement = (_Escapement * 10)
...
_Transparent = ab_transparent
of_SetTransparent(_Transparent)
SetRedraw(TRUE)
RETURN 1
end function
...
//		ll_backcolor2 = of_GetColor(#Backcolor2)
	IF _VGradientFill OR _HGradientFill THEN
		ll_backcolor2 = of_GetColor(_Backcolor2)
	ELSE
